...
sem_t forks[NUM];
sem_t once; // 한 번에 좌, 우 fork를 같이 들기 위한 semaphore
...
void* philosopher(void* arg){
	...
	while(1){
		sem_wait(&once); // deadlock을 피하기 위해 lock 하고 한 번에 양 fork를 동시에 pickup, 점유,대기를 막을 수는 있지만 concurrency를 해친다.
		...
		sem_post(&once); // 이미 두 fork를 픽업하여 점유, 대기 상태가 걸릴 리는 없으니 unlock
